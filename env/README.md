...
### 1.8 提交时约定最晚送到时间（deliver_by）

提交事件时可带 `deliver_by`（ISO 8601 时间，无时区按 UTC 理解），约定**最晚什么时候还能把这一笔送到接收地址**。不带这个字段时照常投递，不受任何影响。

- **过点还没送到的那几家不再给**：到 `deliver_by` 时仍在排队的副本直接置为终态 `deadline_expired`，worker 永不再领取；若 HTTP 请求跨过了这个时间，返回再晚也不会写成 `delivered`。这个状态不同于 `unrouted`（压根没人订）、`filtered`（地址自己的订阅条件没对上）和任何已发状态，trace 里逐地址可查。
- **已经送到的不用收回来**：`delivered_at` 已有值的副本不再被改状态；后来把最晚时间改早或改晚，都只写到当时仍未投妥（`pending` / `in_flight`）的副本。已经因过点关闭的副本也不会因再次修改而复活。
- **已送到的不用收回**：已有 `delivered_at` 的副本保持 `delivered`；即使后来把时间改早，它也不会被改成过点。修改时间时接口返回的 `updated_count` 只统计这次真正被改写的未投妥副本数。
- **最晚时间后来改了，只管还没送到的**：`POST /v1/events/{event_id}/deliver-by`，body 为 `{"deliver_by": "..."}`；传 `null` 表示取消这个约定，仍未投妥的副本恢复照常投递。事件和每份副本都保留当前快照，副本字段为 `deliver_by`，因过点关闭时有 `deliver_by_expired_at`。
- **不能写成已经送到，也不能跟没人订混为一种**：`GET /v1/events/{id}/trace` 的每份 `deliveries` 直接显示该地址是 `delivered` 还是 `deadline_expired`；整笔事件在"无待投副本且至少一份因过点未到"时显示 `deadline_expired`，无人订阅仍是 `unrouted`。事件汇总另有 `deadline_expired_count` / `shadow_deadline_expired_count`。
- **预告正文与接力也守这个点**：预告过点未到，正文一并关闭为 `deadline_expired`；接力中某一站过点未投妥，该站为 `deadline_expired`，后面各站按"停住后不再补"置为 `relay_skipped`（原因 `deliver_by_expired`）。
...
约定最晚送到时间（可选；过点仍未投妥的未发副本停止外发，已投妥的不收回）：

```
body（同样要配合签名头发送）: {
    "event_type": "paid",
    "dedupe_key": "order-1003-paid",
    "deliver_by": "2026-09-10T12:00:00Z",
    "payload": {
      "order_id": "1003",
      "event_type": "paid"
    }
...
- `not_before`：约定的最早外发时间（未定时为 `null`）；`deliver_by`：约定的最晚送到时间（未约定为 `null`）；`cancelled_at`：取消时间（未取消为 `null`）
- `status`：`unrouted`（没有任何地址订该类型）、`pending`（至少一份活跃副本未投完）、`delivered`（全部活跃副本已收到传输层 2xx，但不一定已对上回执）、`cancelled`（未打出前已取消，剩余副本永不再投）、`superseded`（所有副本都因地址换位置而被取代，没有一份打出去，也不补投）、`filtered`（有已确认订户，但每一家自己的订阅条件都没对上这条正文：一份副本都没生成，逐家判定见 trace 的 `filter_evaluations`；它不是 `unrouted`，也不会写成已发）、`dead_lettered`（所有活跃副本都已进死信处、且没有还在排队或投递中的副本；其中任一份被人工复活即回到 `pending`）、`deadline_expired`（没有还在排队的副本，且至少一份因过了 `deliver_by` 还没送到而停止外发；已投妥副本仍保留为 `delivered`）
...
- `delivery_count` / `delivered_count`：**当真**扇出副本总数 / 已投妥数；`deadline_expired_count`：因过了最晚送到时间而停止外发的当真副本数
...
### 取消、改期与修改最晚送到时间（取消/改期仅在任何副本打出之前可用）
...
# 改最晚送到时间：只改还没送到的 pending/in_flight 副本；传 null 取消约定
curl -s -X POST http://localhost:8000/v1/events/<event_id>/deliver-by \
  -H 'Content-Type: application/json' \
  -d '{"deliver_by": "2026-09-10T11:00:00Z"}'
```

取消、改期这两个接口只在所有副本仍是 `pending` 时成功；已有一份副本投妥、正在投，或已经走到不可撤销的终态（如 `deadline_expired`、死信/被取代/接力跳过）时返回 `409`。已取消的事件再改期同样返回 `409`。修改 `deliver_by` 不同：它在副本已投妥后仍可调用，但只改还没投妥的副本，已投妥的不收回、不改写。
